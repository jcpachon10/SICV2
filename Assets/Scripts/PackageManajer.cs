...
public class PackageManajer : MonoBehaviour
{
    ...
    public class PackageType
    {
        ...
        //Methods
        //----------------------------
        public PackageType(Color packageColorP, int packageIdP, Vector3 packageSizeP, Vector3 verticalPosP, Vector3 maxForceP, int quantityP, float weightP, ActualizarPackage uiPackageP, string name)
        {
    ...
    public  void runAlgorithm()
    {
        if (!activeFuntion)
        {
            try
            {
                ...
                //Nombre opcion(1-Clientes(Se pueden apilar) 2-Pesos 3-Pared invisibles)
                startInfo.Arguments = string.Format("{0}/StreamingAssets/Multidrop/{1}",
                UnityEngine.Application.dataPath, "file.txt 1   1   1   3   2  10");
                //StartInfo.Arguments = "Extra Arguments to Pass to the Program";
                startInfo.CreateNoWindow = true;
                startInfo.WindowStyle = ProcessWindowStyle.Hidden;
                //startInfo.UseShellExecute = true;
    ...
    public void newPackage(Vector3 packageSizeP, Vector3 verticalPosP, Vector3 maxForceP, int quantityP, float weightP, bool isEmpty, int Id,int clientID, string name)
    {
        ...
        PackageType p = new PackageType(packageColor[Id], Id, packageSizeP, verticalPosP, maxForceP, quantityP, weightP, uiPackageP, Name.text);
    ...
    */
    ...
    //--------------------------------------------------------------
    public void createNewPackageType(Vector3 packageSizeP, Vector3 verticalPosP, Vector3 maxForceP, int quantityP, float weightP, bool isEmpty, int idP)
    {
        ...
        PackageType p = new PackageType(packageColorP, idP, packageSizeP,verticalPosP, maxForceP, quantityP, weightP, uiPackageP,"item"+idP);
                ...
                try
                {
                    for (int i = 1; i < lines.Length; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(lines[i]))
                        {
                            ...
                            //Cargar datos
                            int packId = int.Parse(split1[0]);
                            Vector3 packSize = new Vector3(int.Parse(split1[3]), int.Parse(split1[4]), int.Parse(split1[5]));
